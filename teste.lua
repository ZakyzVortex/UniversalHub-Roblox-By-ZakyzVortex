...
local AIM_CHECK_ANGLE = true -- Nova opção: só mira em alvos na frente
local AIM_MAX_ANGLE = 90 -- Ângulo máximo (90 = só na frente, 180 = todos os lados)
...
-- NOVA FUNÇÃO: Verifica se o alvo está na frente do jogador
local function isInFrontOfPlayer(targetPart)
    if not AIM_CHECK_ANGLE or not HRP then return true end
    ...
    -- Calcula o ângulo entre a câmera e o alvo (em graus)
    local dotProduct = cameraLookVector:Dot(directionToTarget)
    local angle = math.deg(math.acos(dotProduct))
    
    -- Retorna true se o alvo está dentro do ângulo máximo
...
TabAim:CreateSection("Configurações de Mira")
...
RunService.RenderStepped:Connect(function()
    if AIM_SHOW_FOV and AIM_ENABLED then
...
-- Runtime do Aim Assist (busca de alvos)
RunService.RenderStepped:Connect(function()
        ...
        if player ~= LP and player.Character then
            ...
            if hum and hum.Health > 0 then
                ...
                if targetPart then
                    ...
                    -- Verifica se está na frente (anti-ban)
                    if not isInFrontOfPlayer(targetPart) then
                        continue
                    end
                    
                    -- Verifica se está na tela e dentro do FOV
    ...
    -- Valida se o alvo atual ainda é válido
    if currentTarget then
        ...
        if not parent or not parent:FindFirstChild("Humanoid") then
            ...
            if not hum or hum.Health <= 0 then
                currentTarget = nil
            elseif not isVisible(currentTarget) then
                currentTarget = nil
            elseif not isInFrontOfPlayer(currentTarget) then
                currentTarget = nil
            end
        end
    end
    
    -- Atualiza para o novo alvo mais próximo
...
RunService.RenderStepped:Connect(function()
    if not AIM_ENABLED or not currentTarget or not currentTarget.Parent then
        return
    end
...
-- Cleanup quando o script é destruído
