...
local AIM_CHECK_ANGLE = true
local AIM_MAX_ANGLE = 90
...
-- Função para verificar se o alvo está na frente
local function isInFrontOfPlayer(targetPart)
    if not AIM_CHECK_ANGLE then return true end
    if not targetPart or not targetPart.Parent then return false end
    ...
    return angle <= AIM_MAX_ANGLE
end

-- ==================== UI DO AIM ASSIST ====================

TabAim:CreateSection("Controles Principais")
...
TabAim:CreateToggle({
    ...
    Callback = function(v)
        AIM_ENABLED = v
        currentTarget = nil
        fovCircle.Visible = v and AIM_SHOW_FOV
...
TabAim:CreateToggle({
    Name = "Verificar Ângulo (Anti-Ban)",
    CurrentValue = true,
    Flag = "AimAngleCheck",
    Callback = function(v)
        AIM_CHECK_ANGLE = v
...
TabAim:CreateSection("Círculo FOV")
...
TabAim:CreateToggle({
    ...
    Callback = function(v)
        AIM_SHOW_FOV = v
        fovCircle.Visible = v and AIM_ENABLED
    end
})
...
TabAim:CreateSection("Ajustes de Precisão")
...
-- Loop para atualizar círculo FOV
local aimFOVLoop = RunService.RenderStepped:Connect(function()
    if AIM_SHOW_FOV and AIM_ENABLED and Camera then
...
-- Loop para buscar alvos
local aimTargetLoop = RunService.RenderStepped:Connect(function()
    if not AIM_ENABLED or not HRP or not Camera then
        currentTarget = nil
        return
    end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LP and player.Character then
            ...
            if hum and hum.Health > 0 then
                ...
                if targetPart then
                    -- Wallcheck
                    if AIM_WALLCHECK and not isVisible(targetPart) then
                        continue
                    end
                    
                    -- Angle check
                    if AIM_CHECK_ANGLE and not isInFrontOfPlayer(targetPart) then
                        continue
                    end
        ...
        if not currentTarget.Parent then
            ...
            if not parent:FindFirstChild("Humanoid") then
                ...
                elseif AIM_WALLCHECK and not isVisible(currentTarget) then
                    currentTarget = nil
                elseif AIM_CHECK_ANGLE and not isInFrontOfPlayer(currentTarget) then
                    currentTarget = nil
                end
            end
        end
...
-- Loop de mira suave
local aimCameraLoop = RunService.RenderStepped:Connect(function()
    if not AIM_ENABLED or not currentTarget or not Camera then
        return
    end
    ...
    Camera.CFrame = Camera.CFrame:Lerp(newCFrame, AIM_SMOOTH)
end)

-- Cleanup
local function cleanupAimAssist()
    -- Desconecta loops
    if aimFOVLoop then
        aimFOVLoop:Disconnect()
    end
    if aimTargetLoop then
        aimTargetLoop:Disconnect()
    end
    if aimCameraLoop then
        aimCameraLoop:Disconnect()
    end
...
if typeof(getgenv) == "function" then
    getgenv().CleanupAimAssist = cleanupAimAssist
end

print("✅ Aim Assist Tab carregada - Todas as opções disponíveis!")
...
print("✅ Universal Hub - Organizado e 100% Funcional!")-- ================== UNIVERSAL HUB - ORGANIZED VERSION (FIXED) ==================
-- Universal Hub Rayfield By ZakyzVortex (Mobile Optimized & Organized)
