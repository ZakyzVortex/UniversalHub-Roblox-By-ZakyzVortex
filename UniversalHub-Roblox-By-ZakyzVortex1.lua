...
local function removeESP(player)
    ...
    if espData.line and DRAWING_SUPPORTED then
        pcall(function()
            espData.line:Remove()
        end)
        espData.line = nil
    ...
    if espData.outline and DRAWING_SUPPORTED then
...
local function createESP(player)
    ...
    if NAME_ENABLED or DISTANCE_ENABLED or HEALTH_ENABLED then
        ...
        if success and billboard then
            ...
            espData.txt = billboard:FindFirstChildOfClass("TextLabel")
        end
    end

    -- Linha (só se Drawing for suportado)
    if LINE_ENABLED and DRAWING_SUPPORTED then
    ...
    -- Contorno (só se Drawing for suportado)
    if OUTLINE_ENABLED and DRAWING_SUPPORTED then
        espData.outline = {}
        for i = 1, 4 do
            local success, line = pcall(function()
                local l = Drawing.new("Line")
            ...
            if success then
                table.insert(espData.outline, line)
            end
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if not ESP_ENABLED or not HRP then
        for _, espData in pairs(ESP_OBJECTS) do
            if espData.line and DRAWING_SUPPORTED then 
                pcall(function() espData.line.Visible = false end)
            end
            if espData.outline and DRAWING_SUPPORTED then
                for _, l in ipairs(espData.outline) do
    ...
    for player, espData in pairs(ESP_OBJECTS) do
        ...
        -- Atualiza linha
        if espData.line and LINE_ENABLED and DRAWING_SUPPORTED then
            pcall(function()
        ...
        elseif espData.line and DRAWING_SUPPORTED then
            pcall(function()
                espData.line.Visible = false
            end)
        end
        ...
        -- Atualiza contorno
        if espData.outline and OUTLINE_ENABLED and DRAWING_SUPPORTED then
            pcall(function()
                if onScreen and inFrontOfCamera then
                    local height = 2.5
                    local width = 1.5
        ...
        elseif espData.outline and DRAWING_SUPPORTED then
            pcall(function()
                for _, l in ipairs(espData.outline) do
                    l.Visible = false
                end
...
if DRAWING_SUPPORTED then
    ...
    TabESP:CreateLabel("⚠️ Drawing API não suportada")
    TabESP:CreateLabel("Use Highlight ESP para melhor compatibilidade")
end
...
-- Função para verificar se o inimigo está atrás do jogador
local function isEnemyBehind(targetPart)
    if not HRP then return true end
    
    local lookVector = HRP.CFrame.LookVector
    local toTarget = (targetPart.Position - HRP.Position).Unit
...
TabAim:CreateToggle({
    Name = "Ignorar Inimigos nas Costas",
    CurrentValue = false,
    Callback = function(v)
        AIM_IGNORE_BEHIND = v
        currentTarget = nil
...
print("✅ Universal Hub - Bugs Corrigidos para Arceus X!")
