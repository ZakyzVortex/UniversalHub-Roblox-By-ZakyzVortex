        ...
        if obj:IsA("Model") and obj:FindFirstChildWhichIsA("Humanoid") then
            ...
            if not isPlayer then
                ...
                if hrp and hrp:IsA("BasePart") then
                    ...
                    hrp.Size         = Vector3.new(NPC_HIT_SIZE, NPC_HIT_SIZE, NPC_HIT_SIZE)
                    hrp.Transparency = 0.7
                    hrp.CanCollide   = false
                end
            end
...
local _lastNPCHitboxUpdate = 0
RunService.RenderStepped:Connect(function()
    if not NPC_HIT_ENABLED then return end
    local _now = tick()
    if _now - _lastNPCHitboxUpdate < 0.1 then return end
    _lastNPCHitboxUpdate = _now
    extendNPCHitboxes()
end)

-- ================== PLAYER HITBOX EXTENDER ==================
local PLAYER_HIT_ENABLED = false
local PLAYER_HIT_SIZE    = 10
local originalPlayerHitData = {}

local function extendPlayerHitboxes()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LP and player.Character then
            local hrp = player.Character:FindFirstChild("HumanoidRootPart")
            if hrp and hrp:IsA("BasePart") then
                if not originalPlayerHitData[player.UserId] then
                    originalPlayerHitData[player.UserId] = {
                        Size         = hrp.Size,
                        Transparency = hrp.Transparency,
                        CanCollide   = hrp.CanCollide,
                    }
                end
                hrp.Size         = Vector3.new(PLAYER_HIT_SIZE, PLAYER_HIT_SIZE, PLAYER_HIT_SIZE)
                hrp.Transparency = 0.7
                hrp.CanCollide   = false
            end
        end
...
local function restorePlayerHitboxes()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LP and player.Character then
            local hrp = player.Character:FindFirstChild("HumanoidRootPart")
            local data = originalPlayerHitData[player.UserId]
            if hrp and data then
                pcall(function()
                    hrp.Size         = data.Size
                    hrp.Transparency = data.Transparency
    ...
    originalPlayerHitData = {}
end

Players.PlayerRemoving:Connect(function(player)
    originalPlayerHitData[player.UserId] = nil
end)
...
TabCombat:Toggle({
    ...
    Callback = function(v)
        ...
        if v then extendPlayerHitboxes() else restorePlayerHitboxes() end
    end
})

TabCombat:Slider({
    Title = "Tamanho da Hitbox Player",
    Flag  = "PlayerHitboxSize",
    Step  = 1,
    Value = { Min = 5, Max = 30, Default = 10 },
    Callback = function(v)
        PLAYER_HIT_SIZE = v
        if PLAYER_HIT_ENABLED then extendPlayerHitboxes() end
    end
})

local _lastPlayerHitboxUpdate = 0
RunService.RenderStepped:Connect(function()
    if not PLAYER_HIT_ENABLED then return end
    local _now = tick()
    if _now - _lastPlayerHitboxUpdate < 0.1 then return end
    _lastPlayerHitboxUpdate = _now
    extendPlayerHitboxes()
end)
